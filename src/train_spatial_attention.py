"""
Training script for spatial attention model.

Usage:
    python -m src.train_spatial_attention --epochs 100 --lr 1e-3 --batch_size 128
"""
...
class MultiDigitDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        digits = torch.from_numpy(self.digit_labels[idx]).long()
...
def train_epoch(model, dataloader, criterions, optimizer, device):
    ...
    for imgs, digit_labels, _ in pbar:
        ...
        loss = sum(criterion(output, digit_labels[:, i])
        ...
        pbar.set_postfix({'loss': f'{total_loss/digit_total:.4f}', 'acc': f'{avg_acc:.2f}%'})
...
def evaluate(model, dataloader, criterions, device):
    ...
    return avg_loss, digit_accs, avg_digit_acc, sum_acc
...
def main():
    ...
    if not args.no_wandb:
        wandb.init(
            ...
            name='MultiHeadSpatialAttention',
            config=vars(args)
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, train_digit_accs, train_avg_acc = train_epoch(model, train_loader, criterions, optimizer, device)
        val_loss, val_digit_accs, val_avg_digit_acc, val_sum_acc = evaluate(model, val_loader, criterions, device)
        ...
        if val_sum_acc > best_sum_acc:
            ...
            torch.save(model.state_dict(), f'{args.checkpoint_dir}/spatial_attention_best.pth')
            print(f"✓ Saved best model (sum_acc={val_sum_acc*100:.2f}%)")

    print("\n" + "="*60)
    print(f"Best validation sum accuracy: {best_sum_acc*100:.2f}%")
    print(f"Model saved to: {args.checkpoint_dir}/spatial_attention_best.pth")
